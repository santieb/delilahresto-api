...
const validateChanges = async (req, res, next) => {
  try {
    ...
    } else if (product.name == name && product.abbreviation == abbreviation && product.price == price) 
    return res.status(404).json({ msg: 'you have not made any changes', status: 404 })
